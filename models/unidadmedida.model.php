...
class Unidad_Medida
{
    ...
    public function insertar($Detalle, $Tipo) //insert into Unidad_Medida
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "INSERT INTO `unidad_medida` ( 
			`Detalle`, `Tipo`
			) VALUES (
			'$Detalle', '$Tipo'
			)";
    ...
    public function actualizar($idUnidad_Medida, $Detalle, $Tipo) //update Unidad_Medida 
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "UPDATE `unidad_medida` SET 
			`Detalle`='$Detalle',`Tipo`='$Tipo'
